# GITAM Achievement Portal - Supabase Database Setup

This document contains the SQL commands to set up the database for the GITAM Achievement Portal.
...
-- Trigger to automatically calculate points when achievement is inserted
...
-- Replace 'your-admin-user-id' with actual UUID from auth.users table
INSERT INTO public.users (
    ...
    'your-admin-user-id', -- Replace with actual auth.users.id
    'GITAM Admin',
    'ADM001',
    ...
    'Admin',
    'admin@gitam.edu',
    'hashed_password', -- This should be handled by Supabase Auth
    true,
    true
);
...
2. Disable "Enable email confirmations" for testing (or set up email templates)
3. Set up email templates if you want email confirmations
4. Configure the Auth providers you want to use

## Important Notes

1. Remember to replace 'your-admin-user-id' with actual UUIDs from your auth.users table
2. The password_hash field in the users table is for reference - actual authentication is handled by Supabase Auth
3. You may need to adjust the RLS policies based on your specific security requirements
4. Consider setting up database backups and monitoring
5. Test all policies thoroughly before going to production
...
4. Set up proper email templates for notifications
5. Configure storage bucket permissions as needed
